...
% ── Datos ─────────────────────────────────────────────────────────────────────
\title[Impacto de ChatGPT en el desarrollo de software]{Impacto de la disponibilidad de ChatGPT en el desarrollo de software orientado a Ciencia de Datos (2020--2023)}
...
\begin{block}{Dos fuerzas opuestas sobre el trabajo}
\small La producción es un conjunto de \textbf{tareas} que puede realizar el \textbf{trabajo} (humano) o el \textbf{capital} (algoritmos). Una nueva tecnología empuja en dos direcciones a la vez:
\end{block}

\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{\textcolor{pucpaccent}{Displacement effect}}\\
{\small\textcolor{pucpaccent}{$\downarrow$ reduce la demanda de trabajo}}
\medskip

\small Automatiza tareas antes del programador:
\begin{itemize}\small
...
{\small\textcolor{pucpblue}{$\uparrow$ aumenta la demanda de trabajo}}
\medskip

\small Al abaratar producir código:
\begin{itemize}\small
...
\vspace{0.8em}
\centering \small ¿Cuál fuerza domina? \alert{Nuestra tesis encuentra que domina el \textit{reinstatement}: más desarrolladores, no menos.}
\end{frame}
...
$h_c(z),\ a_c(z)$ & Productividad humana / de ChatGPT en la tarea $z$ \\
$\mu_j,\ A_{it}$ & Acceso individual / disponibilidad país--trimestre \\
$\mathcal{Z}^{\mathrm{auto}}_c,\ \tau_c$ & Conjunto de tareas automatizables y su medida (fracción) \\
...
$\eta,\ r_i,\ \eta^*_c$ & Habilidad, utilidad de reserva y umbral de entrada \\
$F_i,\ f_i$ & Distribución y densidad de habilidad del país \\
$\phi_i=10^5 f_i(\eta^*_c(0))$ & \textbf{Absorción del país} (componente de país) \\
$Y_{ict}$ & \textit{Unique pushers} por 100\,mil habitantes \\
\bottomrule
\end{tabular}
\end{frame}

% ── MT III.2 — El modelo (Pasos 1-2): del individuo al umbral ─────────────────
\begin{frame}{Marco teórico (III): modelo formal de adopción heterogénea}
\framesubtitle{Del desarrollador individual al umbral de entrada (Pasos 1--2)}
\small
\textbf{Paso 1 --- Producción y exposición.} En cada tarea el desarrollador usa el método más productivo (humano o ChatGPT). Por eso su \textbf{ganancia de productividad} al acceder a la herramienta, $\Delta y_{jc}$, proviene \emph{solo} de las tareas que ChatGPT hace mejor; esa ganancia es la \textbf{exposición del lenguaje} $\theta_c$:
\[
\underbrace{\Delta y_{jc}}_{\substack{\text{ganancia de}\\\text{productividad}}}=\underbrace{\tau_c}_{\substack{\text{fracción de tareas}\\\text{automatizables (amplitud)}}}\cdot\underbrace{\rho_c}_{\substack{\text{ahorro relativo}\\\text{promedio (profundidad)}}}\equiv\theta_c.
\]
{\footnotesize Combina \emph{cuántas} tareas mejora ChatGPT ($\tau_c$) y \emph{cuánto} ahorra en ellas ($\rho_c$); es mayor en lenguajes muy representados en los datos de entrenamiento (Python, JavaScript, TypeScript).}

\medskip
\textbf{Paso 2 --- Umbral de entrada.} Cada persona tiene una \textbf{habilidad} $\eta$ (propia de cada individuo, varía entre personas) y entra a programar si supera el \textbf{umbral} $\eta^*_c$ (común a todos: la barra mínima de entrada):
\[
\text{entra si}\quad \underbrace{\eta}_{\substack{\text{habilidad}\\\text{del individuo}}}\ \ge\ \underbrace{\eta^*_c}_{\substack{\text{barra mínima}\\\text{de entrada}}}.
\]
La ganancia de productividad \textbf{baja esa barra} exactamente en $\theta_c$: $\;\eta^*_c(1)=\eta^*_c(0)-\theta_c$.

{\footnotesize Entran quienes tienen habilidad entre la barra nueva y la antigua: los \textit{desarrolladores marginales}.}
\end{frame}

% ── MT III.3 — El modelo (Pasos 3-4): de la agregación al panel ───────────────
\begin{frame}{Marco teórico (III): modelo formal de adopción heterogénea}
\framesubtitle{De la agregación poblacional al panel estimable (Pasos 3--4)}
\small
\textbf{Paso 3 --- Agregación y absorción.} Cuando la barra baja en $\theta_c$, entran las personas que estaban justo debajo. ¿Cuántas? Depende de la \textbf{densidad} de gente cerca del umbral ($f_i$). Por cada 100\,mil habitantes:
\[
\underbrace{\Delta Y_{ic}}_{\substack{\text{nuevos devs.}\\\text{por 100\,mil hab.}}}\approx\; \underbrace{\phi_i}_{\substack{\text{absorción}\\\text{del país}}}\;\cdot\;\underbrace{\theta_c}_{\substack{\text{exposición}\\\text{del lenguaje}}},\qquad \phi_i=10^5 f_i(\eta^*_c(0)).
\]
{\footnotesize Intuición: nuevos $\approx$ (densidad cerca del umbral) $\times$ (cuánto bajó la barra). Por eso $\phi_i$ \emph{es} la densidad $f_i$ evaluada en el umbral.}

\medskip
\textbf{Paso 4 --- Especificación de panel.} Para estimarlo con datos, la variable observada $Y_{ict}$ (\textit{unique pushers}) se descompone en efectos fijos que \textbf{controlan} diferencias permanentes de país ($\alpha_i$), trimestre ($\beta_t$) y lenguaje ($\delta_c$), más el efecto que aparece solo cuando ChatGPT está disponible ($A_{it}=1$):
\[
Y_{ict}=\alpha_i+\beta_t+\delta_c+\underbrace{\textcolor{pucpaccent}{\theta_c\,\phi_i}}_{\text{efecto del tratamiento}}\,A_{it}+\eta_{ict}.
\]
{\footnotesize $A_{it}=1$ si el país $i$ tiene acceso en el trimestre $t$; $\eta_{ict}$: factores no observados. El efecto es \textbf{multiplicativo}: máximo donde coinciden alta exposición y alta absorción.}
\end{frame}

% ── MT III.3 — Supuestos del modelo ──────────────────────────────────────────
\begin{frame}{Marco teórico (III): modelo formal de adopción heterogénea}
\framesubtitle{Supuestos del modelo}
\small
\begin{enumerate}\small
  \item \textbf{Mejor método por tarea:} el desarrollador usa el método más productivo en cada tarea.
  \item \textbf{Covarianza nula:} $\mathrm{Cov}_{\mathcal{Z}^{\mathrm{auto}}_c}(h_c,s_c)=0$ --- la productividad humana y el ahorro relativo no están correlacionados dentro de las tareas automatizables.
  \item \textbf{Normalización de escala:} $\bar h_c=1$ (elección de unidades de producción).
  \item \textbf{Taylor de primer orden:} válido si $\theta_c$ es pequeño frente a la escala en que varía la densidad $f_i$.
  \item \textbf{Separabilidad país--lenguaje:} $\phi_{ic}\approx\phi_i$, lo que permite factorizar el coeficiente como $\theta_c\phi_i$.
  \item \textbf{Exogeneidad condicional:} $\mathbb{E}[\eta_{ict}\mid\alpha_i,\beta_t,\delta_c,A_{it}]=0$ (identificación causal).
\end{enumerate}
\end{frame}

% ── MT III.4 — Conclusiones del modelo ───────────────────────────────────────
\begin{frame}{Marco teórico (III): modelo formal de adopción heterogénea}
\framesubtitle{Conclusiones del modelo}
\small
El efecto de tratamiento es $\Delta Y_{ic}=\theta_c\,\phi_i$. \textbf{Estática comparativa} --- cómo cambia ese efecto al mover cada componente:
\begin{itemize}\small
  \item Más \textbf{exposición} del lenguaje $\Rightarrow$ más efecto: $\;\dfrac{\partial \Delta Y_{ic}}{\partial \theta_c}=\phi_i>0$.
  \item Más \textbf{absorción} del país $\Rightarrow$ más efecto: $\;\dfrac{\partial \Delta Y_{ic}}{\partial \phi_i}=\theta_c>0$.
  \item \textbf{Se refuerzan mutuamente:} $\;\dfrac{\partial^2 \Delta Y_{ic}}{\partial \theta_c\,\partial \phi_i}=1>0$. El efecto de más exposición es mayor cuando la absorción es alta; no se suman, se \emph{multiplican} (complementariedad).
\end{itemize}
\begin{exampleblock}{Predicción contrastable (sustenta la H2)}
\centering\small El ATT debe ser mayor en lenguajes muy expuestos (Python, JavaScript, TypeScript) y en países con más población cerca del umbral de entrada.
\end{exampleblock}
\end{frame}
...
\vspace{1em}
{\Large Quedamos atentos a sus consultas y comentarios.}\par
\vfill
...
\footnotesize
\begin{tabular}{rl}
B.1 & Función de producción a nivel de tarea (Paso 1) \\
B.2 & Conjunto de tareas automatizables (Paso 2) \\
B.3 & Ahorro relativo por tarea (Paso 3) \\
B.4 & Ganancia individual: partición con/sin IA (Paso 4.1--4.2) \\
B.5 & Ganancia exacta y reescritura (Paso 4.3--4.4) \\
B.6 & Identidad de covarianza (Paso 4.5) \\
B.7 & Normalización de escala (Paso 4.6) \\
B.8 & Decisión de entrada al mercado (Paso 5) \\
B.9 & Agregación a desarrolladores activos (Paso 6) \\
B.10 & Per cápita, absorción y separabilidad (Paso 7) \\
B.11 & Especificación de panel con efectos fijos (Paso 8) \\
B.12 & Resumen paso a paso \\
\end{tabular}
\vfill
...
% ── B.1 Paso 1 ────────────────────────────────────────────────────────────────
\begin{frame}{B.1 Función de producción a nivel de tarea (Paso 1)}
\small
...
\end{itemize}
\medskip
{\footnotesize\textit{Regularidad (mantenida en toda la derivación):} $h_c,a_c$ medibles e integrables en $[0,1]$ y $\mathcal{Z}^{\mathrm{auto}}_c$ medible ---para que las integrales existan---; $F_i$ continuamente diferenciable cerca del umbral (necesario en el Paso 6).}
\end{frame}

% ── B.3 Paso 2 ────────────────────────────────────────────────────────────────
\begin{frame}{B.2 Conjunto de tareas automatizables (Paso 2)}
\small
...
% ── B.4 Paso 3 ────────────────────────────────────────────────────────────────
\begin{frame}{B.3 Ahorro relativo por tarea automatizable (Paso 3)}
\small
...
% ── B.5 Paso 4.1–4.2 ──────────────────────────────────────────────────────────
\begin{frame}{B.4 Ganancia individual: partición con/sin IA (Paso 4.1--4.2)}
\small
...
% ── B.6 Paso 4.3–4.4 ──────────────────────────────────────────────────────────
\begin{frame}{B.5 Ganancia exacta y reescritura (Paso 4.3--4.4)}
\small
...
% ── B.7 Paso 4.5 ──────────────────────────────────────────────────────────────
\begin{frame}{B.6 Identidad de covarianza (Paso 4.5)}
\small
...
% ── B.8 Paso 4.6 ──────────────────────────────────────────────────────────────
\begin{frame}{B.7 Normalización de escala (Paso 4.6)}
\small
...
% ── B.9 Paso 5 ────────────────────────────────────────────────────────────────
\begin{frame}{B.8 Decisión de entrada al mercado (Paso 5)}
\small
...
% ── B.10 Paso 6 ───────────────────────────────────────────────────────────────
\begin{frame}{B.9 Agregación a desarrolladores activos (Paso 6)}
\small
...
% ── B.11 Paso 7 ───────────────────────────────────────────────────────────────
\begin{frame}{B.10 Per cápita, absorción y separabilidad (Paso 7)}
\small
...
% ── B.12 Paso 8 ───────────────────────────────────────────────────────────────
\begin{frame}{B.11 Especificación de panel con efectos fijos (Paso 8)}
\small
...
\]
{\footnotesize\textit{Lectura económica:} el efecto de tratamiento $\theta_c\phi_i$ es \textbf{multiplicativo} ---máximo donde coinciden un lenguaje muy expuesto ($\theta_c$ alto) y un país con gran masa cerca del umbral ($\phi_i$ alto); si una de las dos es baja, el efecto se atenúa.}
\end{frame}

% ── B.13 Resumen ──────────────────────────────────────────────────────────────
\begin{frame}{B.12 Resumen paso a paso}
\scriptsize
\renewcommand{\arraystretch}{1.25}
\centering
