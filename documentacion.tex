...
Antes de entrar en los métodos, conviene fijar siete conceptos que vas a ver mencionados muchas veces en la tesis. Si los entiendes acá, no tendrás que buscarlos en internet cada vez que aparezcan. Cada concepto abre con una \textbf{analogía cotidiana} (en azul) para que la intuición entre primero, y recién después viene la definición formal.

\begin{lectura}{Glosario en una frase (para tener a mano)}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}p{4cm}p{9.7cm}@{}}
\textbf{Término} & \textbf{En una frase} \\
\midrule
ATT & Cuánto ayudó el tratamiento a quienes sí lo recibieron. \\
Contrafactual & Qué habría pasado sin el tratamiento (el mundo paralelo que no vemos). \\
Tendencias paralelas & Los dos grupos venían cambiando igual \emph{antes} del tratamiento. \\
Error estándar / IC & El ``$\pm$'' del resultado: cuánta incertidumbre tiene. \\
P-valor & Qué tan raro sería tu resultado si todo fuera puro azar. \\
Bootstrap & Simular muchas veces el estudio barajando los mismos datos. \\
Identificación causal & Probar una \emph{causa}, no solo que dos cosas van juntas. \\
\end{tabular}
\end{lectura}

\subsection*{1. ATT (Average Treatment Effect on the Treated)}

\begin{ideabox}{Analogía: el ensayo de un medicamento}
El ATT responde: ¿cuánto mejoraron los pacientes que \emph{sí} tomaron la pastilla, comparado con cómo habrían estado sin tomarla? Acá la ``pastilla'' es tener ChatGPT disponible y los ``pacientes'' son los 130 países con acceso.
\end{ideabox}
...
\begin{ideabox}{Analogía: el paraguas}
Sales con paraguas y no te mojas. El contrafactual es ``¿qué tan mojado estarías si \emph{no} hubieras llevado paraguas?''. Nunca lo ves directamente —ocurrió en un mundo que no pasó—, pero puedes estimarlo mirando a quienes salieron sin paraguas (el grupo de control).
\end{ideabox}
...
\begin{ideabox}{Analogía: dos autos en la carretera}
Dos autos van separados siempre por 100 metros. Si uno acelera justo cuando le pones un motor nuevo, atribuyes la diferencia al motor. Pero si ya venía acercándose \emph{antes}, no puedes saber si fue el motor o su propia inercia. ``Tendencias paralelas'' significa que los dos grupos venían cambiando de forma pareja antes del tratamiento.
\end{ideabox}
...
\begin{ideabox}{Analogía: el ``$\pm$'' de las encuestas}
Cuando una encuesta dice ``52\,\% $\pm$ 3\,\%'', ese ``$\pm$ 3\,\%'' es la incertidumbre. El error estándar es lo mismo: mide cuánto ``bailaría'' tu resultado si repitieras el estudio con otra muestra de países. El intervalo de confianza es el rango donde es plausible que esté el efecto verdadero.
\end{ideabox}
...
\begin{ideabox}{Analogía: la moneda sospechosa}
Lanzas una moneda 10 veces y salen 10 caras. El p-valor pregunta: ``si la moneda fuera justa (es decir, si el efecto verdadero fuera cero), ¿qué tan raro sería ver algo así?''. Un p-valor chico significa ``sería rarísimo por puro azar'', y eso respalda que el efecto es real y no suerte.
\end{ideabox}
...
\begin{ideabox}{Analogía: repetir el estudio sin repetirlo}
No puedes rehacer el estudio mil veces en el mundo real. El bootstrap lo simula: baraja los mismos países una y otra vez (sacando algunos con repetición), recalcula el efecto en cada baraja, y observa cuánto varía. Esa variación es tu medida de incertidumbre.
\end{ideabox}
...
\begin{ideabox}{Analogía: el gallo y el amanecer}
El gallo canta y luego sale el sol: van juntos (correlación), pero el gallo no causa el amanecer. Para probar una causa necesitas un ``gallo silenciado'' que \emph{no} cante y verificar si el sol igual sale. El grupo de control (países sin ChatGPT) es ese gallo silenciado: si el aumento aparece solo donde llegó ChatGPT y no en los demás, la causa se identifica.
\end{ideabox}
...
\textit{Analogía cotidiana:} piensa en dos tiendas vecinas. A una le pones un cartel nuevo y a la otra no. El cambio de ventas en la tienda \emph{sin} cartel te dice cuánto habrían subido las ventas de todos modos (la tendencia común); el cambio \emph{extra} en la tienda con cartel es el efecto del cartel.
...
\textit{Analogía cotidiana:} es como armar un ``país gemelo'' del grupo tratado con una receta. Si no existe un gemelo natural, lo fabricas mezclando porciones de varios países de control hasta que la mezcla calque la trayectoria previa del tratado —igual que un barista replica un sabor combinando varios granos.
