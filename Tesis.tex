...
La aparición de ChatGPT ha transformado la forma en que los desarrolladores interactúan con herramientas de programación al proporcionar asistencia automatizada para la generación, análisis y depuración de código, reduciendo las barreras de entrada al desarrollo profesional de software. Este estudio evalúa el impacto causal de la disponibilidad de ChatGPT sobre la actividad de desarrollo durante el período Q1-2020--Q4-2023, utilizando la base de datos GitHub Innovation Graph, que registra trimestralmente el número de \textit{unique pushers} por cada 100 mil habitantes a nivel de país y lenguaje de programación. La estrategia empírica explota la variación exógena generada por las decisiones gubernamentales de restringir el acceso al servicio, aplicando los métodos de Diferencias en Diferencias, Control Sintético y Diferencias en Diferencias Sintética (SDID) sobre diez lenguajes de programación. Las tendencias paralelas pretratamiento se validan mediante inspección visual de las trayectorias del contrafactual y gráficos de análisis de evento. Los resultados muestran un impacto positivo y estadísticamente significativo: el estimador SDID documenta incrementos de entre 15\% y 36\% en el número de programadores activos en GitHub respecto al período pretratamiento, con los mayores efectos relativos en TypeScript (35.9\%), Go (32.3\%) y JavaScript (22.7\%). Estos efectos operan principalmente por el margen extensivo, atrayendo nuevos programadores a plataformas colaborativas. La robustez se verifica mediante tres ejercicios complementarios: prueba placebo con tratamiento ficticio en el período pretratamiento, re-estimación con grupo de control restringido que excluye nueve países con restricciones sistémicas de internet y produce resultados cualitativamente idénticos, y análisis de pesos endógenos del estimador SDID.
...
The emergence of ChatGPT has transformed how developers interact with programming tools by providing automated assistance for code generation, analysis, and debugging. This study causally evaluates the impact of ChatGPT availability on software development activity over Q1-2020--Q4-2023, using the GitHub Innovation Graph database, which provides quarterly, country-level records of the number of \textit{unique pushers} per 100,000 inhabitants across programming languages. The empirical strategy exploits the exogenous variation generated by governmental decisions to restrict access to the service, applying Difference-in-Differences, Synthetic Control, and Synthetic Difference-in-Differences (SDID) methods to ten widely used programming languages. Pre-treatment parallel trends are validated through visual inspection of counterfactual trajectories and event study graphs. Results show a positive and statistically significant impact: the SDID estimator documents increases of between 15\% and 36\% in the number of active GitHub developers relative to the pre-treatment baseline, with the largest relative effects in TypeScript (35.9\%), Go (32.3\%), and JavaScript (22.7\%). These effects operate primarily through the extensive margin, reducing barriers to entry into software development and attracting new programmers to collaborative platforms. Robustness is further verified through three complementary exercises: a placebo test with fictitious treatment assigned in the pre-treatment period, a re-estimation with a restricted control group excluding nine countries with systematic internet restrictions that yields qualitatively identical results, and an analysis of the endogenous unit weights of the SDID estimator.
...
La estrategia de identificación de este estudio se basa en la variación exógena generada por las decisiones gubernamentales de restringir el acceso a ChatGPT. La variable de tratamiento $W_{it}$ toma valor 1 para los países con disponibilidad del servicio a partir del cuarto trimestre de 2022. Esta variación presenta dos características favorables para la identificación causal: (i) el inicio del tratamiento es simultáneo para todas las unidades tratadas (diseño de bloque), lo que elimina la heterogeneidad en el momento de adopción y simplifica la construcción del contrafactual; y (ii) las restricciones gubernamentales obedecen a decisiones regulatorias, geopolíticas y de seguridad nacionales previas al lanzamiento del servicio, lo que reduce el riesgo de endogeneidad respecto a la actividad de desarrollo en GitHub.

El supuesto central de identificación es la \textit{tendencia paralela condicional}: en ausencia de ChatGPT, la evolución del número de \textit{unique pushers} en el grupo tratado habría seguido una trayectoria comparable a la del contrafactual sintético, construido mediante las ponderaciones SDID. Este supuesto es validado empíricamente mediante dos estrategias complementarias: (i) la inspección visual de las trayectorias pretratamiento en las Figuras 5--14, que muestran un ajuste cercano entre el grupo tratado y el control sintético en los 12 trimestres anteriores a Q4-2022; y (ii) los gráficos de análisis de evento del Anexo C, donde los \textit{gaps} período a período entre el grupo tratado y el control sintético se concentran estadísticamente en torno a cero durante el período pretratamiento.

La principal amenaza a la identificación es la no-aleatoriedad del grupo de control: los 29 países sin acceso a ChatGPT no lo bloquearon por razones aleatorias. Un subconjunto de ellos ---clasificado como \textit{Not Free} con puntaje inferior a 25 en el índice \textit{Freedom on the Net} 2022 de Freedom House (China, Rusia, Irán, Cuba, Bielorrusia, Siria, Tayikistán, Vietnam y Turkmenistán)--- presenta restricciones sistémicas de internet, ecosistemas tecnológicos estructuralmente distintos y menores niveles de actividad en GitHub, lo que puede dificultar la comparación con el grupo tratado.

Para cuantificar en qué medida estos países influyen en la construcción del contrafactual, se extrajeron los pesos $\omega$ asignados por el estimador SDID a cada unidad de control en las diez especificaciones de lenguaje. Estos pesos se presentan en el Cuadro~6 (Sección de Tablas). Los resultados revelan un patrón heterogéneo: China (CN) recibe el peso promedio más bajo del grupo (0.019), notablemente inferior al promedio del resto de controles (0.032), en línea con la singularidad estructural de su ecosistema tecnológico centrado en Gitee. En contraste, Bielorrusia (BY\,=\,0.076), Rusia (RU\,=\,0.071) y Vietnam (VN\,=\,0.068) reciben pesos superiores al promedio, lo que refleja que sus trayectorias de actividad en GitHub son cuantitativamente comparables con las del grupo tratado en el período pretratamiento. Este hallazgo indica que SDID los utiliza como comparadores genuinamente válidos; la prueba decisiva es la especificación con grupo de control restringido, donde se re-estima el modelo excluyendo explícitamente los nueve censores severos.

Como verificación adicional, se re-estimó el modelo SDID excluyendo explícitamente los nueve censores severos del grupo de control (Cuadro 5). Los resultados son cualitativamente idénticos a la especificación principal, con diferencias porcentuales de magnitud reducida, lo que confirma que las estimaciones no dependen de estos países. Por último, se implementó una prueba placebo con tratamiento ficticio en Q2-2021 (véase Cuadro 4), cuyo análisis se discute en la sección de resultados.
...
El \hyperref[tab:tabla3]{Cuadro 3} presenta las estimaciones del efecto del tratamiento para los tres métodos. El resultado más destacable es la consistencia de signo: en los diez lenguajes analizados, los tres estimadores arrojan efectos positivos, lo que refleja un patrón robusto de mayor actividad en GitHub tras la disponibilidad de ChatGPT. En términos absolutos, los lenguajes con mayor base de usuarios registran los incrementos más elevados: JavaScript lidera con un ATT-SDID de 8.303 \textit{unique pushers} adicionales por cada 100 mil habitantes, seguido por Python (4.525) y TypeScript (3.078). Los lenguajes de sistemas (C, C++) y PHP muestran efectos absolutos menores pero estadísticamente significativos y consistentes entre los tres estimadores. La única excepción es C\#, donde el estimador SDID no alcanza significancia estadística (0.443, SE\,=\,0.464), aunque DiD y SC sí lo hacen (1.499 y 1.255, respectivamente). Esta divergencia sugiere sensibilidad al supuesto de tendencias paralelas para ese lenguaje; es notable que en la especificación con grupo de control restringido (Cuadro~5), el efecto para C\# sí resulta significativo (0.965**), lo que indica que parte de la imprecisión puede atribuirse a la influencia de unidades de control estructuralmente distintas.

Estos resultados son coherentes con un mecanismo de \textit{margen extensivo}: la asistencia en codificación que provee ChatGPT reduce las barreras de entrada al desarrollo de software, facilitando que programadores con menor experiencia inicien o retomen su actividad en plataformas colaborativas como GitHub. La variable dependiente ---\textit{unique pushers}, el número de desarrolladores únicos que realizan al menos un \textit{push} en el trimestre--- captura precisamente esta expansión del conjunto de participantes activos, y no un aumento en la intensidad del trabajo de quienes ya eran usuarios habituales. Este canal de incorporación es especialmente activo en lenguajes con comunidades en rápido crecimiento, alta representación en los datos de entrenamiento del modelo y documentación abundante en línea, factores que amplifican la utilidad de ChatGPT como asistente de aprendizaje.
...
En términos de magnitud relativa, los efectos SDID estimados son económicamente significativos. Tomando como referencia los promedios pretratamiento de cada lenguaje (\textit{Baseline Mean} en el Cuadro 3), los efectos representan incrementos porcentuales sustanciales: TypeScript registra el mayor aumento relativo (35.9\%), seguido por Go (32.3\%), Python (22.6\%), JavaScript (22.7\%) y C (20.5\%). Los lenguajes con menor efecto relativo son Java (15.0\%), PHP y Ruby (14.4\% cada uno) y C++ (16.6\%). C\#, cuyo estimador SDID no resulta significativo, registra un efecto relativo de apenas 6.8\%. El mayor efecto relativo en TypeScript y Go, frente al menor en PHP y los lenguajes de sistemas maduros (C, C++), es coherente con la hipótesis de que la magnitud del impacto es proporcional al margen potencial de nuevos usuarios: los lenguajes con mayor crecimiento de demanda en el mercado laboral concentran una mayor proporción de programadores que se inician con ayuda de herramientas de IA generativa.

Como validación adicional de la estrategia de identificación, el Cuadro 4 presenta los resultados de la prueba placebo, en la que el tratamiento se asigna de forma ficticia en Q2-2021 restringiendo la muestra a los 11 trimestres pretratamiento. Los resultados muestran un patrón informativo: para cinco lenguajes (C\#, C++, Java, PHP, Ruby) los estimadores placebo son estadísticamente indistinguibles de cero, confirmando la ausencia de efectos espurios. Para los cinco restantes (C, Go, JavaScript, Python, TypeScript) se detectan efectos placebo positivos pero de magnitud sustancialmente menor que los reales: el cociente entre el ATT placebo y el ATT real promedia 0.38, lo que indica que aproximadamente el 62\% del efecto estimado es atribuible al tratamiento y no a tendencias preexistentes. Esta diferencia en magnitud, combinada con la coincidencia temporal entre el lanzamiento de ChatGPT y la aceleración observada en las trayectorias post-Q4-2022, refuerza la interpretación causal de los resultados principales.
...
\subsection{Análisis de robustez e identificación}

\noindent\textbf{Pesos $\omega$ del estimador SDID para los censores severos.}
Un argumento central para confiar en la estrategia de identificación es que el propio estimador SDID pondera internamente las unidades de control según su capacidad de replicar las trayectorias pretratamiento del grupo tratado. El Cuadro~6 presenta los pesos $\omega$ asignados a los nueve países con restricciones sistémicas de internet (Freedom on the Net 2022 $<$ 25) en las diez especificaciones de lenguaje. El patrón es heterogéneo: China (CN) recibe el peso más bajo del grupo (promedio\,=\,0.019, frente a 0.032 del resto de controles), consistente con la singularidad de su ecosistema tecnológico centrado en Gitee. En contraste, Bielorrusia (BY\,=\,0.076), Rusia (RU\,=\,0.071) y Vietnam (VN\,=\,0.068) concentran pesos por encima del promedio del grupo de control, lo que indica que sus trayectorias de actividad en GitHub son cuantitativamente comparables con las del grupo tratado durante el período pretratamiento. Este hallazgo implica que el estimador los utiliza como comparadores válidos; la verificación directa de que su inclusión no distorsiona los resultados se realiza mediante la especificación con grupo de control restringido (Cuadro~5).

\noindent\textbf{Especificación con grupo de control restringido.}
Como verificación explícita, el Cuadro 5 re-estima el modelo SDID excluyendo los nueve censores severos del grupo de control, reduciéndolo de 29 a 20 países. Las estimaciones resultantes son cualitativamente idénticas a la especificación principal: la dirección del efecto se mantiene en los diez lenguajes, la significancia estadística es equivalente en todos los casos donde el modelo principal era significativo, y las diferencias porcentuales entre ambas especificaciones son de magnitud reducida. Este resultado confirma que los hallazgos principales no dependen de las unidades de control más problemáticas y son robustos a la composición del grupo de comparación.

\noindent\textbf{Prueba placebo.}
El Cuadro 4, discutido en la sección anterior, complementa el análisis mostrando que al asignar el tratamiento de forma ficticia en el período pretratamiento, los efectos estimados son sustancialmente menores (cociente promedio 0.38 respecto a los efectos reales), lo que descarta que los resultados principales sean explicados por tendencias preexistentes.

En conjunto, los tres ejercicios de validación ---pesos omega endógenos, grupo de control restringido, y prueba placebo--- articulan una estrategia de identificación multicapa que aborda directamente las amenazas más relevantes a la validez causal del diseño.

\clearpage
\section{Conclusiones y recomendaciones de política}

En conclusión, los resultados obtenidos mediante el modelo SDID indican que la disponibilidad de ChatGPT en la mayoría de los países del mundo ha tenido un impacto positivo y diferenciado en el desarrollo de software en varios lenguajes de programación. Específicamente, se observó un aumento notable en la cantidad de desarrolladores que utilizan lenguajes como JavaScript, Python, Java y TypeScript. Estos hallazgos sugieren que la asistencia proporcionada por ChatGPT motiva el desarrollo de software en estos lenguajes. Por otro lado, C\# presenta un patrón heterogéneo entre estimadores: DID y SC arrojan efectos positivos y significativos (1.499 y 1.255 \textit{unique pushers}, respectivamente), mientras que el estimador SDID, que impone las tendencias paralelas de forma más estricta, no alcanza significancia estadística (0.443). Esta divergencia sugiere que la estimación del impacto en C\# es sensible al supuesto de tendencias paralelas, lo que motiva interpretarla con cautela. Este impacto sobre el desarrollo de software muestra variaciones significativas entre diferentes contextos nacionales también resalta que el impacto de esta herramienta no es uniforme. Factores como la infraestructura tecnológica, la adopción de tecnologías y las políticas de apoyo al desarrollo de software pueden desempeñar un papel crucial en la determinación de la magnitud del impacto.
...
En primer lugar, la composición del grupo de control es la principal amenaza a la validez interna del diseño. Los países que bloquearon ChatGPT no lo hicieron de forma aleatoria: muchos presentan mayor control estatal sobre internet y ecosistemas tecnológicos estructuralmente distintos (ej. China, Rusia, Irán). Esta no-aleatoriedad es abordada mediante tres estrategias complementarias presentadas en la Sección de Resultados: (i) el análisis de pesos $\omega$ endógenos del SDID (Cuadro~6), que revela un patrón heterogéneo ---China recibe el peso más bajo (0.019), mientras que Bielorrusia, Rusia y Vietnam obtienen pesos por encima del promedio del grupo de control, lo que indica que sus trayectorias en GitHub son genuinamente comparables con las del grupo tratado---; (ii) la re-estimación con grupo de control restringido (Cuadro~5, sin los 9 países con Freedom on the Net $<$ 25), que produce resultados cualitativamente idénticos a la especificación principal; y (iii) la prueba placebo con tratamiento ficticio (Cuadro~4). A pesar de estas mitigaciones, no es posible descartar completamente que diferencias estructurales no capturadas por las trayectorias pretratamiento afecten las estimaciones.
...
En cuarto lugar, la divergencia entre métodos observada para algunos lenguajes merece atención. En Ruby, el DiD no resulta significativo (0,262) mientras que el CS y el SDiD sí lo son (0,662 y 0,808, respectivamente), lo que sugiere que el control sintético construye un mejor contrafactual para ese lenguaje. En C\#, ocurre el patrón inverso: el DiD y el CS son significativos (1,499 y 1,255), pero el SDiD no alcanza significancia (0,443). Esta heterogeneidad entre estimadores puede indicar sensibilidad en la construcción del contrafactual o variabilidad en la asignación de pesos, y merece análisis adicional.
...
\input{output/tables/placebo_test.tex}
\input{output/tables/robustness_restricted_control.tex}
\input{output/tables/omega_weights_table.tex}
...
\label{fig:mapa_gpt}
\includegraphics[width=\textwidth]{output/figures/chatgpt_global_availability_map.png}
\end{figure}
...
BB & Barbados & ET & Ethiopia \\
BE & Belgium & IR & Iran, Islamic Republic of \\
BZ & Belize & LA & Lao People's Democratic Republic \\
BJ & Benin & LY & Libya \\
BT & Bhutan & MO & Macao \\
BO & Bolivia, Plurinational State of & PR & Puerto Rico \\
BA & Bosnia and Herzegovina & RU & Russian Federation \\
BW & Botswana & RE & Réunion \\
BR & Brazil & SO & Somalia \\
BN & Brunei Darussalam & SD & Sudan \\
BG & Bulgaria & SY & Syrian Arab Republic \\
BF & Burkina Faso & TJ & Tajikistan \\
CV & Cabo Verde & TM & Turkmenistan \\
CA & Canada & TR & Türkiye \\
CL & Chile & UZ & Uzbekistan \\
CO & Colombia & VE & Venezuela, Bolivarian Republic of \\
CG & Congo & VN & Viet Nam \\
CR & Costa Rica & YE & Yemen \\
HR & Croatia & ZW & Zimbabwe \\
CY & Cyprus &  &  \\
CZ & Czechia &  &  \\
CI & Côte d'Ivoire &  &  \\
DK & Denmark &  &  \\
...
Las variables de control empleadas en la estimación con controles (Cuadro 7) fueron extraídas del \textbf{Banco Mundial} y corresponden a dos indicadores de infraestructura digital:
...
\midrule
0 & 4640 & 1160 & 1160 & 3480 \\
1 & 20800 & 5200 & 5200 & 15600 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption*{\textit{Missing values o datos vacíos por país}}
\begin{tabular}{lcc}
\toprule
